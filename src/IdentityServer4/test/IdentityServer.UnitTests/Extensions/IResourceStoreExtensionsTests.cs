...
namespace IdentityServer.UnitTests.Extensions
{
    public class IResourceStoreExtensionsTests
    {
        ...
        public async Task GetAllEnabledResourcesAsync_on_duplicate_identity_scopes_should_fail()
        {
            ...
            var exception = await a.Should().ThrowAsync<Exception>();
            exception.And.Message.ToLowerInvariant().Should().Contain("duplicate");
            exception.And.Message.ToLowerInvariant().Should().Contain("identity scopes");
        }
        ...
        public async Task GetAllEnabledResourcesAsync_on_duplicate_api_resources_should_fail()
        {
            ...
            var exception = await a.Should().ThrowAsync<Exception>();
            exception.And.Message.ToLowerInvariant().Should().Contain("duplicate");
            exception.And.Message.ToLowerInvariant().Should().Contain("api resources");
        }
